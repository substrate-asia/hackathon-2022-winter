...
contract AVaultForStarlayUpgradable is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20PermitUpgradeable {
    ...
    function initialize(
        ...
        require(_decimals == IERC20MetadataUpgradeable(_wantAddress).decimals(), "decimal"); // should ajust the scale if not equal.
    ...
    function getNetAssetValue(ILendingPool _lendingPool) public view returns(uint){
        ...
        uint lTokenBalance = IERC20Upgradeable(reserve.lTokenAddress).balanceOf(address(this));
        uint variableDebt = IERC20Upgradeable(reserve.variableDebtTokenAddress).balanceOf(address(this));
        uint wantBalance = IERC20Upgradeable(wantAddress).balanceOf(address(this));
    ...
    function _farm() internal virtual {
        ...
        uint256 wantAmt = IERC20Upgradeable(_wantAddress).balanceOf(address(this));
        if(wantAmt > 0){
            ILendingPool lendingPool = getLendingPool();
            IERC20Upgradeable(_wantAddress).safeIncreaseAllowance(address(lendingPool), wantAmt);
            lendingPool.deposit(_wantAddress, wantAmt, address(this), 0);
        }
    }

    function ajustToTargetLTV() external nonReentrant{
        _ajustLTV(0);
    }
    ...
    function _ajustLTV(uint _toWithdrawAmount) internal{
        ...
        uint _lTokenBalance = IERC20Upgradeable(reserve.lTokenAddress).balanceOf(address(this));
        uint _variableDebt = IERC20Upgradeable(reserve.variableDebtTokenAddress).balanceOf(address(this));
        ...
        if(_lTokenBalance * _targetLTV > _variableDebt * _ltvScale + (_targetLTV * _toWithdrawAmount)){
            ...
            for(uint i = 0; i < _maxLoopCount && _deltaOfLoan > 0; i++){
                ...
                IERC20Upgradeable(_wantAddress).safeIncreaseAllowance(address(_lendingPool), _toWithdraw);
        ...
        nonReentrant
    {
        ...
        IERC20Upgradeable(_wantAddress).safeTransfer(_userAddress, _wantAmt);
    }
    ...
    function _earn() internal virtual  {
        ...
        IZap _zap = zap;
        IERC20Upgradeable(_earnedAddress).safeIncreaseAllowance(address(_zap), earnedAmt);
    ...
    function buyBack(uint256 _earnedAmt) internal virtual returns (uint256 remainAmt) {
        ...
        if(interimBuybackHolder != address(0)){
            IERC20Upgradeable(_earnedAddress).transfer(interimBuybackHolder, _buyBackAmt);
        }else{
            address _buybackLP = buybackLP;
            IERC20Upgradeable(_earnedAddress).safeIncreaseAllowance(address(zap), _buyBackAmt);
            zap.zapInToken(_earnedAddress, _buyBackAmt, _buybackLP);

            uint _lpBalance = IERC20Upgradeable(_buybackLP).balanceOf(address(this));
            if(_lpBalance > 0){
                IERC20Upgradeable(_buybackLP).transfer(address(buyBackAddress), _lpBalance);
    ...
    function approveRewardHanlder(address _rewardHandler, address _rewardToken) external virtual onlyOwner{
        ...
        IERC20Upgradeable(_rewardToken).safeIncreaseAllowance(_rewardHandler, type(uint).max);
