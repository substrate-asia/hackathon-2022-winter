...
func newKeyring(address string, pubkey [32]byte, privkey [64]byte) (*keyring, error) {
	addrPubKey, err := DecodeAddressToPublicKey(address)
	if err != nil {
		return nil, err
	}
	if addrPubKey != "0x"+hex.EncodeToString(pubkey[:]) {
...
func SignWithNonStore(address, pubKey, privkey, data string) ([]byte, error) {
	var (
		privateKey [64]byte
		publicKey  [32]byte
	)
	priv, err := types.HexDecodeString(privkey)
	if err != nil {
		return nil, err
	}
	pub, err := types.HexDecodeString(pubKey)
	if err != nil {
		return nil, err
	}
	...
	kr, err := newKeyring(address, publicKey, privateKey)
	if err != nil {
		return nil, err
	}
