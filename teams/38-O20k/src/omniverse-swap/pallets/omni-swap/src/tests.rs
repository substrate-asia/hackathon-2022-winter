...
fn it_works_for_add_liquidity() {
	new_test_ext().execute_with(|| {
		...
		let data_x =
			encode_transfer(&secp, (secret_key, public_key), public_key, 100000000000000, nonce);
		let data_y =
			encode_transfer(&secp, (secret_key, public_key), public_key, 1000000000000, nonce);
		assert_ok!(
		...
		assert_eq!(
			OmniSwap::trading_pairs(trading_pair.clone()),
			Some((100000000000000, 1000000000000))
		);
