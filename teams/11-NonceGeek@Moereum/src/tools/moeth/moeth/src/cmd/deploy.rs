...
#[derive(Parser, Debug)]
#[clap(about = "moeth deploy [FILE_NAME|PATH] [OPTIONS]
    Examples:
    $ moeth deploy
...
impl Deploy {
    pub fn apply(&mut self, ctx: &mut Context) -> Result<()> {
        // Run `moeth package build` first to build all necessary artifacts.
...
pub fn run_moeth_package_build(ctx: &mut Context) -> Result<()> {
