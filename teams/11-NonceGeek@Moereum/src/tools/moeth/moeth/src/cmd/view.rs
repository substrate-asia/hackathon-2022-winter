...
#[derive(Parser, Debug)]
#[clap(about = "moeth view [QUERY] [OPTIONS]
    Examples:
    $ moeth view Account::Store::U64
    $ moeth view Account::Store::U64 --api http://127.0.0.1:9933
    $ moeth view Account::Store::U64 --api http://127.0.0.1:9933 --json
    $ moeth view 0x1::Account::Balance<0x1::Coins::ETH> --api http://127.0.0.1:9933 --json --output PATH/SAVE.json
")]
pub struct View {
    #[clap(
        display_order = 1,
...
impl View {
    pub fn apply(&mut self, ctx: &mut Context) -> anyhow::Result<()> {
        ...
        let block_num = self.block_num.clone();
        ...
        match query {
            TypeTag::Struct(st) => {
                ...
                net.get_resource(&addr, &st, &block_num)
                    .map(|resp| {
                        let view = NetView::new(net, block_num);
                        if let Some(bytes_for_block) = resp {
                            ...
                            annotator
                                ...
                                .and_then(|result| {
                                    let block_num = bytes_for_block.1;

                                    if self.json {
                                        serde_json::ser::to_string_pretty(
                                            &ser::AnnotatedMoveStructWrapper { height: block_num, result },
                                        )
