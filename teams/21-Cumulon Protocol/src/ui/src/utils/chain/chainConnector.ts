...
export class ChainConnector {
  ...
  async getCurrentRoundInfo(): Promise<any> {
    ...
    let formatSymbolDecimals = new BigNumber('1e' + AppConfig.ParaChainSymbolDecimals)
  ...
  async getSelectedCollators4CurrentRound(): Promise<any> {
    ...
    if (candidates) {
      for (let index = 0; index < candidates.length; index++) {
        ...
        let convertedAddress = ChainUtils.ss58transform_simple(c, AppConfig.ParaChainName);
  ...
  async getRealtimeCollatorCandidatePool(): Promise<any> {
    ...
    for (let index = 0; index < cps.length; index++) {
      const cp = cps[index];
      let convertedAddress = ChainUtils.ss58transform_simple(cp.owner, AppConfig.ParaChainName);
  ...
  constructor() {
    this.logger = new W3Logger('ChainConnector');
  }

  async init(wssEndpoint: string, modulePrefix: string) {
    this.wssEndpoint = wssEndpoint;
    this.modulePrefix = modulePrefix;
    this.logger.modulePrefix = modulePrefix;
    this.logger.debug('init wssEndpoint:' + wssEndpoint);
    const types = Object.values(definitions).reduce(
      (res, { types }): object => ({ ...res, ...types }),
      {},
    );
    this.wsProvider = new WsProvider(this.wssEndpoint);
  ...
  async checkReady() {
    ...
    if (isReady) {
      ...
      await this.init(this.wssEndpoint, this.modulePrefix);
    }
  }
