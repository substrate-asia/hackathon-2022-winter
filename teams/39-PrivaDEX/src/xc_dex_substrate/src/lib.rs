...
mod privadex_xc_dex {
    use super::*;
    use substrate_utils::chain_info::{self, ChainToken};
    use chain_info::{
        AddressType, ChainInfo, RelayChain, SignatureScheme, Ss58AddressFormat, xcm_prelude, universal_chain, UniversalChainId
    };
    ...
    impl PrivaDex {
        ...
        pub fn initiate_xc_transfer(
            &self,
            src_network_name: String,
            src_token: ChainToken,
            src_amount: u128,
            dest_network_name: String,
        ) -> Result<ERC20XCSwapStateTransition /* ::XCTransferInitiated */> {
            let privkey = self.escrow_private_key.ok_or(Error::UninitializedEscrow)?;

            let src_chain_info = chain_info::get_chain_info(&src_network_name).ok_or(Error::UnknownChain)?;
            let dest_chain_info = chain_info::get_chain_info(&dest_network_name).ok_or(Error::UnknownChain)?;
            let dest_addr = self.get_escrow_account_pubkey(&dest_chain_info)?;
            
            let versioned_multiasset: xcm_prelude::VersionedMultiAsset = 
                src_token.to_versioned_multiasset(src_chain_info.chain_id, src_amount).ok_or(Error::UnsupportedToken)?;
            let versioned_multilocation: xcm_prelude::VersionedMultiLocation =
                chain_info::dest_to_versioned_multilocation(&src_chain_info, &dest_chain_info, &dest_addr).map_err(
                    |_| Error::NoPathFromSrcToDest
                )?;

            let encoded_call_data = match src_chain_info.chain_id {
                universal_chain::MOONBEAM | UniversalChainId::SubstrateParachain(RelayChain::MoonbaseRelay, _) =>
                    Ok(extrinsic_call_factory::moonbeam_xtokens(versioned_multiasset, versioned_multilocation)),
                _ => Err(Error::UnsupportedChain),
            }?;

            let subutils = SubstrateUtils{ rpc_url: src_chain_info.rpc_url.clone() };
            let sender = self.get_escrow_account_pubkey(&src_chain_info)?;
            let nonce = subutils.get_next_nonce(&slice_to_hex_string(&sender)).map_err(|_| Error::RequestFailed)?;
            let runtime_version = subutils.get_runtime_version().map_err(|_| Error::RequestFailed)?;
            let genesis_hash = subutils.get_genesis_hash().map_err(|_| Error::RequestFailed)?;
            let era = Era::Immortal;
            let finalized_head = if era != Era::Immortal {
                subutils.get_finalized_head_hash().map_err(|_| Error::RequestFailed)?
            } else { genesis_hash.clone() };

            let tx_raw = match src_chain_info.sig_scheme.get_address_type() {
                AddressType::Ethereum => {
                    let signer: [u8; 20] = sender.try_into().map_err(|_| Error::InvalidAddress)?;
                    let sigconfig = ExtrinsicSigConfig::<[u8; 20]>{
                        sig_scheme: SignatureScheme::Ethereum,
                        signer,
                        privkey: privkey.to_vec(),
                    };
                    subutils.create_extrinsic::<[u8; 20]>(
                        sigconfig,
                        &encoded_call_data,
                        nonce,
                        runtime_version,
                ...
                AddressType::SS58 => {
                    let signer: [u8; 32] = sender.try_into().map_err(|_| Error::InvalidAddress)?;
                    let sigconfig = ExtrinsicSigConfig::<[u8; 32]>{
                        sig_scheme: SignatureScheme::Sr25519,
                        signer,
                        privkey: privkey.to_vec(),
                    };
                    subutils.create_extrinsic::<[u8; 32]>(
                        sigconfig,
                        &encoded_call_data,
                        nonce,
                        runtime_version,
            ...
            let extrinsic_hash = subutils.send_extrinsic(&tx_raw).map_err(|_| Error::TransactionFailed)?;
            Ok(ERC20XCSwapStateTransition::XCTransferInitiated(src_network_name, dest_network_name, self.cur_block(), extrinsic_hash))
        }
        ...
        pub fn eth_swap_upon_xc_transfer_completed(
            ...
            let txn_hash = match src_token {
                ...
                ChainToken::ERC20(src_token_eth_addr) => dex_router.swap_exact_tokens_for_tokens(
                    amount_in.into(),
                    0.into() /* amount_out_min */,
                    vec![src_token_eth_addr, dest_token_eth_addr],
                    escrow_addr /* to */,
                    deadline_millis.into(),
                    &key,
                ).map_err(|_| Error::TransactionFailed)?,
        ...
        pub fn erc20_transfer(
            ...
            let contract = eth_utils::ERC20Contract::new(&chain_info.rpc_url, token).map_err(|_| Error::RequestFailed)?;
            let txn_hash = contract.transfer(dest_addr, amount.into(), &key).map_err(|_| Error::RequestFailed)?;
            Ok(ERC20XCSwapStateTransition::ERC20Transfer(network_name, txn_hash))
        }
    ...
    mod tests {
        ...
        fn get_phat_contract() -> Addressable<PrivaDex> {
            ...
            let _ = contract.call_mut().init_secret_keys(kap_privkey.0).expect("Valid init");
            contract
        }
