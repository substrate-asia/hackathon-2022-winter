...
type FileClient interface {
	...
	SendFileSt(fid string, c local_cache.Cacher) error
}
...
func (c *ConMgr) handler(cache local_cache.Cacher) error {
	var (
		err      error
		recvFile *os.File
	)
...
func NewFileClient(conn NetConn, dir string, files []string) FileClient {
	return &ConMgr{
		conn:       conn,
		dir:        dir,
		sendFiles:  files,
...
func (c *ConMgr) SendFileSt(fid string, cache local_cache.Cacher) error {
	c.conn.HandlerLoop()
	go func() {
		_ = c.handler(cache)
	}()
...
func (c *ConMgr) recvFile(fid string, fsize int64, pkey, signmsg, sign []byte) error {
	...
	utils.Infof("Ready to receive head: %s", fid)
	...
	tmpFp, err := os.Create(filepath.Join(c.dir, fid))
	if err != nil {
		c.conn.SendMsg(NewCloseMsg(fid, Status_Err))
		return err
	}
	utils.Infof("Created tmp file %s", tmpFp.Name())
...
func (c *ConMgr) sendSingleFile(filePath string, fid string, fsize int64, lastmark bool, pkey, signmsg, sign []byte) error {
	...
	log.Printf("Ready to write file: %s, file info: %+v", filePath, fileInfo)
